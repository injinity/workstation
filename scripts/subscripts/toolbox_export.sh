...
desktop_file_to_export=$(toolbox run -c injinity find /usr/share/applications -iname "*$app_name*.desktop" 2>/dev/null)

# Find icons associated with the application
icons_to_export=$(toolbox run -c injinity find /usr/share/icons/hicolor/**/apps -iname "*$app_name*" 2>/dev/null)
...
if [ -z "$desktop_file_to_export" ]; then
  ...
  toolbox run -c injinity mkdir -pv ~/.local/share/applications/
  toolbox run -c injinity cp -v "$desktop_file_to_export" ~/.local/share/applications/
  ...
  toolbox run -c injinity sed -i "s|^Exec=.*|$new_exec|" "$desktop_file"
...
if [ -z "$icons_to_export" ]; then
  ...
  toolbox run -c injinity mkdir -pv ~/.local/share/icons/hicolor
  ...
  for icon in $icons_to_export; do
    ...
    toolbox run -c injinity mkdir -pv "$HOME/.local/share/icons/hicolor/$target_dir"
    # Copy the icon to the new location
    toolbox run -c injinity cp -v "$icon" "$HOME/.local/share/icons/hicolor/$target_dir/"
  done
fi
